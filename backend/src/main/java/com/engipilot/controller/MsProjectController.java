...
public class MsProjectController {
    ...
    public ResponseEntity<Map<String, Object>> parse(
        ...
        String ext = nom.contains(".")
                ? nom.substring(nom.lastIndexOf('.') + 1).toLowerCase()
                : "";
        ...
        try {
            ...
            for (Task task : project.getTasks()) {
                ...
                if (Boolean.TRUE.equals(task.getSummary())) continue;
                ...
                t.put("progress",
                        task.getPercentageComplete() != null
                                ? task.getPercentageComplete().intValue()
                                : 0
                );

                // ✅ FIX MPXJ SAFE (remplace toInstant / toLocalDate)
                t.put("startDate",
                        task.getStart() != null ? task.getStart().toString() : null
                );

                t.put("endDate",
                        task.getFinish() != null ? task.getFinish().toString() : null
                );
