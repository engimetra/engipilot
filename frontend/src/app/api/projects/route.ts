...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const body = JSON.stringify({
      codeProjet:         raw.reference ?? raw.codeProjet,
      nom:                raw.name      ?? raw.nom,
      budgetPrevisionnel: raw.budgetInitial ?? raw.budgetPrevisionnel,
      dateDebut:          raw.startDate ?? raw.dateDebut,
      dateFinPrevue:      raw.endDate   ?? raw.dateFinPrevue,
      ville:              raw.city      ?? raw.ville,
      client:             raw.clientName ?? raw.client,
