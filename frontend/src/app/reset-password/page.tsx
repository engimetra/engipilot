...
export default function ResetPasswordPage() {
  ...
  useEffect(() => { if (!token) setError("Lien invalide. Veuillez refaire la demande.") }, [token])
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1e512d]/10 to-white px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
        ...
        <p className="text-sm text-gray-500 text-center mb-6">Choisissez un mot de passe sécurisé d&#39;au moins 8 caractères</p>
        {success ? (
          <div className="flex flex-col items-center gap-3 py-6">
            <CheckCircle2 className="w-12 h-12 text-[#1e512d]" />
            <p className="text-center text-gray-700 font-medium">Mot de passe mis à jour !</p>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="password" type={showPass ? "text" : "password"} value={password}
                  ...
                  className="w-full pl-9 pr-10 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e512d]" required />
                <button type="button" onClick={() => setShowPass(p => !p)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                  {showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
              {password && !passwordOk && <p className="text-xs text-red-500 mt-1">Minimum 8 caractères requis</p>}
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input id="confirm" type={showPass ? "text" : "password"} value={confirm}
                  ...
                  className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e512d]" required />
              </div>
              {confirm && !match && <p className="text-xs text-red-500 mt-1">Les mots de passe ne correspondent pas</p>}
            </div>
            <button type="submit" disabled={loading || !passwordOk || !match || !token}
              className="w-full py-2.5 rounded-lg bg-[#1e512d] text-white font-semibold text-sm hover:bg-[#1e512d]/90 disabled:opacity-50 flex items-center justify-center gap-2 mt-2">
