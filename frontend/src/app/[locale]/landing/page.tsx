    ...
    <div className="min-h-screen bg-[#F7F9FC] text-[#0B132B]">
        ...
        <div className="max-w-7xl mx-auto px-6" ref={navRef}>
          <div className="flex items-center justify-between h-[72px]">
                  ...
                  <div key={item.label} className="relative">
                    <button
                      ...
                      className={`flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                        isOpen ? "bg-[#F0F7F2] text-[#1e512d]" : "text-gray-600 hover:text-[#0B132B] hover:bg-gray-50"
                      }`}
                    >
                      {item.label}
                      {hasDropdown && (
                        <svg className={`w-3.5 h-3.5 transition-transform duration-200 ${isOpen ? "rotate-180 text-[#1e512d]" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button onClick={() => router.push("/login")} className="text-sm font-medium text-gray-600 hover:text-[#1e512d] px-3 py-2 rounded-lg hover:bg-gray-50 transition-all">
                Se connecter
              </button>
              <button onClick={() => router.push("/login")} className="flex items-center gap-2 bg-[#1e512d] hover:bg-[#174024] transition-all text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-[0_2px_12px_rgba(30,81,45,0.35)] hover:shadow-[0_4px_20px_rgba(30,81,45,0.45)] hover:-translate-y-0.5">
            ...
            <button className="lg:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu">
          ...
          {mobileOpen && (
            <div className="lg:hidden border-t border-gray-100 pb-4 pt-2 space-y-1">
                  ...
                  {mobileExpanded === item.label && item.dropdown && (
                    <div className="ml-3 mt-1 space-y-0.5 border-l-2 border-[#E4F0E7] pl-3">
                      {item.dropdown.columns.flatMap(c => c.items).map((it) => (
                        <button key={it.title} onClick={() => { setMobileOpen(false); if (it.scroll) scroll(it.scroll); }} className="w-full flex items-center gap-2 px-2 py-2 text-sm text-gray-600 hover:text-[#1e512d] rounded-lg hover:bg-[#F0F7F2] transition-colors text-left">
              ...
              <div className="pt-3 border-t border-gray-100 flex flex-col gap-2 px-1">
                <button onClick={() => router.push("/login")} className="w-full py-2.5 text-sm font-medium text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                  Se connecter
                </button>
                <button onClick={() => router.push("/login")} className="w-full py-2.5 text-sm font-bold text-white bg-[#1e512d] rounded-xl hover:bg-[#174024] transition-colors">
                  Commencer gratuitement →
                </button>
              </div>
            </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="mt-10 text-lg text-gray-500 leading-relaxed max-w-xl">
              ENGIPILOT centralise la gestion de vos chantiers, équipes, documents et performances dans une plateforme collaborative boostée par l'IA.
            </p>
            <div className="mt-10 flex gap-4 flex-wrap">
              <button onClick={() => router.push("/login")} className="flex items-center gap-2 bg-[#1e512d] hover:bg-[#174024] transition-all text-white px-8 py-4 rounded-2xl font-bold shadow-[0_4px_24px_rgba(30,81,45,0.4)] hover:shadow-[0_8px_32px_rgba(30,81,45,0.5)] hover:-translate-y-0.5">
                Commencer gratuitement
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button>
              <button onClick={() => scroll("features")} className="flex items-center gap-2 border border-gray-200 bg-white px-8 py-4 rounded-2xl font-semibold hover:bg-gray-50 hover:border-gray-300 transition-all hover:-translate-y-0.5 shadow-sm">
            ...
            <div className="relative rounded-[28px] overflow-hidden shadow-2xl border border-gray-100" style={{ aspectRatio: "4/3" }}>
              ...
              <div className="absolute bottom-4 left-4 right-4 grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={k.label} className="bg-white/90 backdrop-blur-sm rounded-2xl px-3 py-2.5 shadow-md flex items-center gap-2.5">
      ...
      <section id="solutions" className="py-12 border-y bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-gray-400 text-xs uppercase tracking-[0.2em] mb-10 font-semibold">Ils nous font confiance</p>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="mt-5 text-gray-500 text-lg max-w-2xl mx-auto">
              ENGIPILOT analyse vos données en temps réel pour prédire les risques, optimiser les ressources et améliorer les performances de vos chantiers.
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: "🔮", title: "Prédiction des retards",   desc: "L'IA détecte les dérives de planning 3 semaines à l'avance et propose des actions correctives.", badge: "IA",        color: "#6366f1" },
              { icon: "📊", title: "Tableaux de bord EVM",     desc: "Indicateurs SPI, CPI et valeur acquise calculés automatiquement pour chaque projet.",               badge: "Analytics", color: "#1e512d" },
              { icon: "⚠️", title: "Détection d'anomalies",    desc: "Alertes proactives sur les risques qualité, sécurité et budgétaires en temps réel.",                badge: "Alertes",   color: "#f59e0b" },
              { icon: "💰", title: "Optimisation des coûts",   desc: "Réductions moyennes de 34 % sur les surcoûts grâce à l'analyse prédictive des dépenses.",           badge: "Finance",   color: "#ff751f" },
              { icon: "🤝", title: "Assistant intelligent",    desc: "Un copilote IA répond à vos questions, génère des rapports et optimise votre planning.",             badge: "Copilote",  color: "#ec4899" },
              { icon: "📋", title: "Rapports automatisés",     desc: "Rapports journaliers, hebdomadaires et mensuels générés automatiquement et envoyés par email.",       badge: "Auto",      color: "#14b8a6" },
            ].map((f) => (
              <div key={f.title} className="bg-white border border-gray-100 rounded-3xl p-8 hover:shadow-xl hover:border-gray-200 transition-all duration-300 hover:-translate-y-1 group">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl" style={{ background: f.color + "15" }}>{f.icon}</div>
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full border" style={{ color: f.color, borderColor: f.color + "40", background: f.color + "10" }}>{f.badge}</span>
                </div>
          ...
          <div className="text-center mt-12">
            <button onClick={() => router.push("/login")} className="inline-flex items-center gap-2 bg-[#1e512d] hover:bg-[#174024] text-white px-8 py-4 rounded-2xl font-bold shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5">
      ...
      <section id="modules" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-5xl font-black">Une plateforme complète pour tous vos besoins</h2>
            <p className="mt-5 text-gray-500 text-lg">Des modules intégrés pour gérer chaque aspect de vos projets de construction</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
            {modules.map((module) => (
              <button key={module.title} onClick={() => router.push("/login")} className="bg-[#F7F9FC] border border-transparent rounded-2xl p-6 hover:bg-white hover:border-[#1e512d]/20 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 text-left group">
      ...
      <section id="tarifs" className="py-24 bg-[#F7F9FC]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="mt-5 text-gray-500 text-lg">Commencez gratuitement · Passez à Pro quand vous êtes prêt</p>
          </div>
              ...
              <div key={plan.name} className={`relative rounded-3xl border flex flex-col gap-6 overflow-hidden transition-transform hover:-translate-y-1 ${plan.highlight ? "shadow-2xl scale-[1.02]" : "bg-white border-gray-200 shadow-sm hover:shadow-md"}`}>
                {plan.highlight && (
                  <>
                    ...
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#ff751f] text-white text-xs font-bold px-4 py-1 rounded-full shadow-lg">LE PLUS POPULAIRE</div>
                  </>
                )}
                <div className="relative p-8 flex flex-col gap-6 flex-1">
                  <div>
                    ...
                    <div className="mt-3 flex items-end gap-1">
                      <span className={`text-4xl font-black ${plan.highlight ? "text-white" : "text-[#1e512d]"}`}>{plan.price}</span>
                      {plan.sub && <span className={`text-sm mb-1 ${plan.highlight ? "text-white/60" : "text-gray-400"}`}>{plan.sub}</span>}
                    </div>
                  </div>
                  ...
                  <button onClick={() => router.push("/login")} className={`w-full py-3.5 rounded-2xl font-bold transition-all hover:scale-[1.02] text-sm ${plan.highlight ? "bg-white text-[#1e512d] hover:bg-[#f0fdf4] shadow-[0_4px_20px_rgba(0,0,0,0.15)]" : "text-white bg-gradient-to-r from-[#1e512d] to-[#2d7a42] hover:from-[#174024] hover:to-[#245f35] shadow-md"}`}>
                    {plan.cta}
                  </button>
                </div>
              </div>
          ...
          <p className="text-center mt-10 text-sm text-gray-400">Tous les prix sont HT · Essai gratuit sans carte bancaire · Annulation à tout moment</p>
        </div>
      </section>
      ...
      <section id="ressources" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { icon: "📖", title: "Documentation",   desc: "Guides complets, tutoriels pas à pas et référence API exhaustive.",                   cta: "Consulter la doc",       color: "#1e512d" },
              { icon: "🎬", title: "Vidéos tutoriels",desc: "Apprenez ENGIPILOT en moins de 15 minutes avec nos vidéos guidées.",                 cta: "Regarder",               color: "#6366f1" },
              { icon: "📊", title: "Modèles Excel",   desc: "Templates d'import chantiers prêts à l'emploi, formats CSV et XLSX.",                 cta: "Télécharger",            color: "#ff751f" },
              { icon: "🎧", title: "Support expert",  desc: "Notre équipe technique répond en moins de 2 h, 7 j/7.",                              cta: "Contacter le support",   color: "#ec4899" },
            ].map((r) => (
              <div key={r.title} className="bg-[#F7F9FC] border border-transparent rounded-3xl p-8 hover:bg-white hover:border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col gap-4 group">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl" style={{ background: r.color + "15" }}>{r.icon}</div>
                ...
                <button onClick={() => scroll("ressources")} className="text-sm font-bold hover:underline text-left flex items-center gap-1 transition-colors" style={{ color: r.color }}>
                  {r.cta}
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                  </svg>
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-gray-600 text-lg leading-relaxed mb-5">
              ENGIPILOT est né d'un constat simple : les professionnels du BTP méritent des outils à la hauteur de leurs défis. Notre plateforme combine la puissance de l'IA avec une expérience utilisateur pensée pour le terrain.
            </p>
            <p className="text-gray-600 text-lg leading-relaxed mb-10">
              Développée par des ingénieurs passionnés de construction et de technologie, ENGIPILOT accompagne aujourd'hui plus de 2 400 chantiers dans 15 pays.
            </p>
            <div className="flex gap-4">
              <button onClick={() => router.push("/login")} className="flex items-center gap-2 bg-[#1e512d] hover:bg-[#174024] text-white px-6 py-3.5 rounded-2xl font-bold transition-all hover:-translate-y-0.5 shadow-lg">
                Rejoindre ENGIPILOT
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button>
              <button onClick={() => scroll("a-propos")} className="flex items-center gap-2 border border-gray-200 bg-white px-6 py-3.5 rounded-2xl font-semibold transition-all hover:-translate-y-0.5 hover:border-gray-300 shadow-sm">
                Nous contacter
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="relative bg-gradient-to-br from-[#0f2e18] to-[#1e512d] rounded-[40px] p-16 overflow-hidden text-center">
            ...
            <div className="relative">
              ...
              <h2 className="text-5xl font-black text-white leading-tight">Prêt à transformer la gestion<br />de vos projets BTP ?</h2>
              <p className="mt-6 text-xl text-white/60 max-w-2xl mx-auto">Rejoignez les entreprises qui font confiance à ENGIPILOT pour piloter leurs projets avec succès.</p>
              <div className="mt-10 flex gap-4 justify-center flex-wrap">
                <button onClick={() => router.push("/login")} className="flex items-center gap-2 bg-white text-[#1e512d] px-8 py-4 rounded-2xl font-black hover:bg-[#f0fdf4] transition-all hover:-translate-y-0.5 shadow-[0_4px_24px_rgba(255,255,255,0.25)]">
                  Commencer gratuitement
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                  </svg>
                </button>
                <button onClick={() => scroll("a-propos")} className="flex items-center gap-2 border border-white/30 text-white px-8 py-4 rounded-2xl font-semibold hover:bg-white/10 transition-all hover:-translate-y-0.5">
                  Planifier une démo
                </button>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-5 gap-12 pb-12 border-b border-white/10">
            <div className="lg:col-span-2">
              ...
              <p className="text-white/50 leading-relaxed text-sm max-w-xs">La plateforme tout-en-un pour piloter vos projets BTP avec intelligence et performance.</p>
              <div className="mt-6 flex gap-3">
                {["in", "tw", "yt"].map((s) => (
                  <button key={s} className="w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center text-xs font-bold text-white/60 hover:text-white">{s}</button>
                ))}
              </div>
            </div>
            {[
              { title: "Produit",    links: [{ label: "Fonctionnalités", scroll: "features" }, { label: "Modules", scroll: "modules" }, { label: "Tarifs", scroll: "tarifs" }, { label: "Nouveautés", scroll: "hero" }] },
              { title: "Ressources", links: [{ label: "Documentation", scroll: "ressources" }, { label: "Vidéos", scroll: "ressources" }, { label: "Blog", scroll: "ressources" }, { label: "Support", scroll: "ressources" }] },
            ].map((col) => (
              <div key={col.title}>
                ...
                <ul className="space-y-3">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <button onClick={() => scroll(link.scroll)} className="text-white/50 hover:text-white text-sm transition-colors">{link.label}</button>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div>
              ...
              <div className="flex rounded-xl overflow-hidden border border-white/10">
                <input type="email" placeholder="Votre email" className="flex-1 px-4 py-3 text-sm bg-white/5 text-white placeholder-white/30 outline-none focus:bg-white/10 transition-colors" />
                <button type="button" className="bg-[#ff751f] hover:bg-[#e8611a] px-4 transition-colors text-white font-bold text-sm">→</button>
              </div>
            </div>
          </div>
...
// DEPLOY-TEST-1781881690
// DEPLOY-TEST-1781882215
