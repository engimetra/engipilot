...
function buildKpis(data?: DashboardKpis): KPICardProps[] {
  ...
  return [
    { icon: Building2,  label: "Active Projects",  sublabel: "Company sites",         value: String(data.activeProjects), delta: `${data.totalProjects} total`, deltaUp: true,      accent: "#635BFF", accentBg: "bg-primary/10", accentText: "text-primary", sparkline: [1,1,1,2,2,2,3,data.activeProjects].map(v=>Math.max(v,1)), href: "/chantiers" },
    { icon: TrendingUp, label: "Global Progress",  sublabel: "Portfolio average",      value: `${data.avgProgress}%`,      delta: data.avgSpi!==null?`SPI ${data.avgSpi.toFixed(2)}`:undefined, deltaUp: spiGood, accent: "#FDAB3D", accentBg: "bg-warning/10", accentText: "text-warning", sparkline: [data.avgProgress-8,data.avgProgress-6,data.avgProgress-4,data.avgProgress-2,data.avgProgress].map(v=>Math.max(v,1)), href: "/analytics" },
    { icon: Wallet,     label: "Budget Consumed",  sublabel: "Across all projects",    value: `${data.budgetPct}%`,        delta: data.budgetPct>75?"Attention budget":"Dans les limites", deltaUp: data.budgetPct<=75, accent: "#E2445C", accentBg: "bg-danger/10", accentText: "text-danger", sparkline: [data.budgetPct-12,data.budgetPct-9,data.budgetPct-6,data.budgetPct-3,data.budgetPct].map(v=>Math.max(v,1)), href: "/analytics" },
    { icon: Clock,      label: "Critical Delays",  sublabel: "Sites requiring action", value: String(data.criticalDelays), delta: data.criticalDelays>0?"Action requise":"Tout en ordre", deltaUp: data.criticalDelays===0, accent: "#E2445C", accentBg: "bg-danger/10", accentText: "text-danger", sparkline: [0,0,0,0,0,0,0,Math.max(data.criticalDelays,0)].map(v=>Math.max(v,1)), href: "/planning" },
    { icon: ShieldAlert,label: "HSE Incidents",    sublabel: "Open incidents",         value: String(data.incidentCount),  delta: data.incidentCount===0?"No active incidents":"Under investigation", deltaUp: data.incidentCount===0, accent: "#14b8a6", accentBg: "bg-teal/10", accentText: "text-teal", sparkline: [0,0,0,0,0,0,0,Math.max(data.incidentCount,0)].map(v=>Math.max(v,1)), href: "/hse" },
    { icon: Bell,       label: "Notifications",    sublabel: "Non lues",               value: String(data.notifCount),     delta: data.notifCount>0?`${data.notifCount} en attente`:"Tout lu", deltaUp: data.notifCount===0, accent: "#8b5cf6", accentBg: "bg-purple/10", accentText: "text-purple", sparkline: [0,0,0,0,0,0,0,Math.max(data.notifCount,0)].map(v=>Math.max(v,1)), href: "/notifications" },
  ]
}
